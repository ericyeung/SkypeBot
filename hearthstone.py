...
def getCardDescription(name):
	...
	if isinstance(data,list):
		for item in data:
			if 'imgGold' in item and 'cost' in item: #A gold version of a card ensures that card is indeed a card and not an enchantment.
				name = item['name'] if 'name' in item else ''
				# Replacements of characters are for special formating characters in the text that we do not need
				text = item['text'].replace("<b>", '').replace('</b>', '').replace('$', '').replace('#', '') if 'text' in item else ''
				img = item['imgGold'] if 'imgGold' in item else ''
				cost = str(item['cost']) + " Mana" if 'cost' in item else ''
				stats = str(item['attack']) + "/" + str(item['health']) if ('attack' in item and 'health' in item) else ''
